...
- `Photo` — Id, UserId (FK→User, cascade), Ordinal, IsPrimary, Width, Height, StorageKey (e.g. `users/{userId}/photos/{photoId}` — variants live at `{key}/thumb.webp|card.webp|full.webp`), ContentType, ModerationStatus (`approved` by default; full mod queue is Tier-6 BACKLOG), UploadedAt. Unique on `(UserId, Ordinal)`. **Partial unique index** on `(UserId, IsPrimary) WHERE IsPrimary = true` enforces "at most one primary per user." Up to 6 photos per user (enforced in `UploadPhotoHandler`).
...
- **Endpoints** (`Application/Photos/PhotosController`, `[Authorize]`, route `/api/users/me/photos`):
  - `POST /api/users/me/photos` (multipart `file=...`) → returns `PhotoDto` (with signed URLs).
  - `DELETE /api/users/me/photos/{photoId}` → removes blob prefix + row; if it was primary, promotes the next photo (lowest Ordinal) to primary.
  - `PUT /api/users/me/photos/{photoId}/primary` → two-phase: clear current primaries first, then set new (keeps the filtered unique index happy).
- **`UserProfileDto.Photos`** is `IReadOnlyList<PhotoDto>` ordered by `Ordinal`; **`PrimaryPhotoUrl`** is the *card-size* signed URL for the primary photo (or null). **`MatchProfileDto.PrimaryPhotoUrl`** mirrors that — same card-size URL for the candidate user, populated by a single bulk query in `GetMatchesHandler` / `GetFestivalAttendeesHandler` (`db.LoadPrimaryPhotoCardUrlsAsync(...)`).
