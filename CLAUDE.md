...
- **Discovery ordering**: `GetDiscoveryHandler` orders candidates by `(SharedFestivalCount DESC, SharedArtistCount DESC, LastActiveAt DESC)`. Shared festival count is the intersection of the current user's `Going|Ticketed` festivals with the candidate's; shared artist count is the top-artist intersection. `CommonFestivals` on `MatchProfileDto` carries the festival *names* of that intersection — clients display them directly. **No physical-distance signal** (see `docs/MATCHING.md`).
...
- **No user geolocation**: user→user physical distance was removed from matching (2026-06-01) — the geography that matters is shared festivals, not home city. `User` has no `Location`; only `City` (display text). `Festival.Location` (geography) remains for festival info/maps. PostGIS extension stays enabled for it. (Historical gotcha, still true if you ever reintroduce a distance query: compute `ST_Distance` **inside** the EF query — `Point.Distance` after `ToListAsync` returns degrees, not meters.)
